...
public:
	...
	BOOL Create(HWND pWnd /*= NULL*/)
	{
	   ...
	   m_bitmap.Attach( (HBITMAP)handle );
		...
		return (BOOL)ParentClass::Create(pWnd,CRect(0,0,bm.bmWidth, bm.bmHeight),NULL,WS_POPUP | WS_VISIBLE);
	}
                   ...
                   LPARAM lParam, BOOL& bHandled)
	{
		...
		bmp.CreateCompatibleBitmap( dc.m_hDC, m_ImgSize.cx, m_ImgSize.cy );
		memDC.SelectBitmap( bmp.m_hBitmap );
		memDC.DrawState( point, m_ImgSize, m_bitmap.m_hBitmap, DST_BITMAP | DSS_NORMAL );
		dc.BitBlt( 0, 0, rc.right, rc.bottom, memDC.m_hDC,0, 0, SRCCOPY );
		Sleep(2000);
		return 0;
	}
