<!--
/*
基于[WEB Calendar ver 3.0 作者：meizz(梅花雪疏影横斜)]改写，主要改动三处：
1、对全局内容进行判断，以允许一个页面多次包含此脚本文件；
2、将嵌在用户页面的HTML代码改用insertAdjacentHTML方法插入BODY中，并采用异步方式初始化，原来的document.write要求脚本全局代码
在实际页面内容被输出前执行，因为它会清除页面中的内容（除write,writeln的内容外）
3、删除getObjectById函数，改用document.getElementById取对象,这使用对IE版本要求在5.0以上
by alan 2005.12.23
*/
if(typeof(_include_calendar_once_)=='undefined')
{
	_include_calendar_once=true;
	function registerCalendar()
	{
		if(!document.body)
			setTimeout(registerCalendar,200);
		else
		{
			document.body.insertAdjacentHTML("afterBegin","<div id=meizzCalendarLayer style='position: absolute; z-index: 9999; width: 144; height: 193; display: none'>"+
				"<iframe name=meizzCalendarIframe scrolling=no frameborder=0 width=100% height=100%></iframe></div>");
			defCalendar();
		}
	}
	function defCalendar()
	{
		try
		{
			WebCalendar = new TWebCalendar();
		}
		catch(eee)
		{
			setTimeout(defCalendar,200);
		}
	}
	registerCalendar();
