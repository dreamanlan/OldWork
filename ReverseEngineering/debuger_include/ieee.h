...
idaman void ida_export emovo(const eNI a, eNE b);    // move eNI => eNE
idaman void ida_export emovi(const eNE a, eNI b);    // move eNE => eNI
...
//
idaman int  ida_export emdnorm(eNI s, int lost, int subflg, int32 exp, int rndbase);
//
...
idaman int ida_export ieee_realcvt(void *m, eNE e, uint16 swt);
...
idaman void ida_export realtoasc(const eNE x, char *buf, size_t bufsize, uint mode);
...
idaman void ida_export eltoe64(int64 l, eNE e);                  // int64 to IEEE
idaman void ida_export eltoe64u(uint64 l, eNE e);                // uint64 to IEEE
idaman int ida_export eetol(eNE a, sval_t *l, bool roundflg);    // IEEE to long (+-0.5 if flg)
idaman int ida_export eetol64(eNE a, int64 *l, bool roundflg);   // IEEE to long (+-0.5 if flg)
idaman int ida_export eetol64u(eNE a, uint64 *l, bool roundflg); // IEEE to ulong (+-0.5 if flg)
// exponent
idaman int ida_export eldexp(const eNE a, int32 pwr2, eNE b);           // b = a*(2**pwr2)
// arifmetic operations
idaman int ida_export eadd(const eNE a, const eNE b, eNE c, int subflg);     // if(!subflg) c = a + b
                                                                 // else        c = a - b
idaman int ida_export emul(const eNE a, const eNE b, eNE c);                 // c = a * b
idaman int ida_export ediv(const eNE a, const eNE b, eNE c);                 // c = a / b
// predefined function
...
// comparison
idaman int ida_export ecmp(const eNE a, const eNE b); // 0  if a = b
                                                      // 1  if a > b
                                                      // -1 if a < b
