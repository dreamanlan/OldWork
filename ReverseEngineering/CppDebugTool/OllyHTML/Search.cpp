...
int Hex2Bin(const CString &s, char* arr, DWORD size)
{
	int i = 0;
	ZeroMemory(arr, size);
	while(i < s.GetLength() / 2)
	{
		arr[i] = (BYTE) strtoul(s.Mid(i * 2, 2), 0, 16);
		i++;
	}
	return i;
}
...
int FindWithWildcards(const char* source, const char* findstring, int len)
{
	char cmp[3] = {0};
	int findlen = (int)ceil(((double)strlen(findstring) / 2));
	if(len < findlen)
		return -1;
...
bool Replace(char * s, const char * searchstr, const char * replstring, int length)
{
	char * ps=s;
	...
	if (((int)strlen(searchstr)/2)>length)
