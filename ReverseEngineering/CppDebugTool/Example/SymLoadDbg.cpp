...
//   * Load/unload symbols for a module
//   * Display information about the kind of symbols loaded for a module
//   * Receive and display the troubleshooting messages from the symbol 
//     engine (DbgHelp.dll)
//
...
// Usage:
//
// SymLoadDbg -p <pid>   Attach to the specified process
// SymLoadDbg <CmdLine>  Launch the specified executable, with the specified 
...
int _tmain( int argc, TCHAR* argv[] )
{
	...
	DWORD Timeout = 2000; // in milliseconds, INFINITE is also possible
	...
	while( bContinue ) 
	{
		...
		if( WaitForDebugEvent( &DebugEvent, Timeout ) )
		{
			...
			case CREATE_PROCESS_DEBUG_EVENT:
				...
				// Handle the event (manually break it into the series of three events)
...
void CDebugger::OnCreateProcessEvent( DWORD ProcessId )
{
	...
	m_SymbolEngine.AddOptions( SYMOPT_DEBUG | SYMOPT_LOAD_LINES );
		...
		// -> Set it to "false" if you want to suppress the messages

	if( !m_SymbolEngine.Init( m_hProcess, 0, false, true ) )
...
{
	_tprintf( _T("Usage:\n") );
	_tprintf( _T("  SymLoadDbg -p <pid>   attach to the process\n") );
	_tprintf( _T("  SymLoadDbg <CmdLine>  launch the process\n") );
	_tprintf( _T("\n") );
}

void PrintLogo()
{
	_tprintf( _T("SymLoadDbg sample application\n") ); 
